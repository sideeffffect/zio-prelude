...
import zio.prelude.coherent.DeriveParEqualTraversable
import zio.prelude.newtypes.{ And, First, Max, Min, Or, Prod, Sum }
import zio.test.laws._
import zio.{ Chunk, ChunkBuilder, NonEmptyChunk }

/**
 * `ParTraversable` is an abstraction that describes the ability to iterate over
 ...
 */
trait ParTraversable[F[+_]] extends Traversable[F] {
   ...
   * of the effect.
   */
  def foreachPar[G[+_]: IdentityBoth: Covariant, A, B: Associative](fa: F[A])(f: A => G[B]): G[F[B]]

  /**
   * Returns whether the collection contains the specified element.
   */
  override def contains[A, A1 >: A](fa: F[A])(a: A1)(implicit A: Equal[A1]): Boolean =
    exists(fa)(_ === a)
   ...
   * specified prediate.
   */
  override def count[A](fa: F[A])(f: A => Boolean): Int =
   ...
   * predicate.
   */
  override def exists[A](fa: F[A])(f: A => Boolean): Boolean =
    foldMap(fa)(a => Or(f(a)))
...
object ParTraversable extends LawfulF.Covariant[DeriveEqualParTraversable, Equal] {

  /**
   * The set of all laws that instances of `ParTraversable` must satisfy.
   */
  val laws: LawsF.Covariant[DeriveEqualParTraversable, Equal] =
    Covariant.laws

  /**
   * Summons an implicit `ParTraversable`.
   */
  def apply[F[+_]](implicit traversable: ParTraversable[F]): ParTraversable[F] =
    traversable
}

trait ParTraversableSyntax {

  /**
   * Provides infix syntax for traversing collections.
   */
  implicit class ParTraversableOps[F[+_], A](private val self: F[A]) {
    def foreach[G[+_]: IdentityBoth: Covariant, B](f: A => G[B])(implicit F: ParTraversable[F]): G[F[B]] =
      F.foreach(self)(f)
    def contains[A1 >: A](a: A1)(implicit A: Equal[A1], F: ParTraversable[F]): Boolean                   =
      F.contains[A, A1](self)(a)
    def count(f: A => Boolean)(implicit F: ParTraversable[F]): Int                                       =
      F.count(self)(f)
    def exists(f: A => Boolean)(implicit F: ParTraversable[F]): Boolean                                  =
      F.exists(self)(f)
    def find(f: A => Boolean)(implicit F: ParTraversable[F]): Option[A]                                  =
      F.find(self)(f)
    def foldLeft[S](s: S)(f: (S, A) => S)(implicit F: ParTraversable[F]): S                              =
      F.foldLeft(self)(s)(f)
    def foldMap[B: Identity](f: A => B)(implicit F: ParTraversable[F]): B                                =
      F.foldMap(self)(f)
    def foldRight[S](s: S)(f: (A, S) => S)(implicit F: ParTraversable[F]): S                             =
      F.foldRight(self)(s)(f)
    def forall(f: A => Boolean)(implicit F: ParTraversable[F]): Boolean                                  =
      F.forall(self)(f)
    def foreach_[G[+_]: IdentityBoth: Covariant](f: A => G[Any])(implicit F: ParTraversable[F]): G[Unit] =
      F.foreach_(self)(f)
    def isEmpty(implicit F: ParTraversable[F]): Boolean                                                  =
      F.isEmpty(self)
    def mapAccum[S, B](s: S)(f: (S, A) => (S, B))(implicit F: ParTraversable[F]): (S, F[B])              =
      F.mapAccum(self)(s)(f)
    def maxOption(implicit A: Ord[A], F: ParTraversable[F]): Option[A]                                   =
      F.maxOption(self)
    def maxByOption[B: Ord](f: A => B)(implicit F: ParTraversable[F]): Option[A]                         =
      F.maxByOption(self)(f)
    def minOption(implicit A: Ord[A], F: ParTraversable[F]): Option[A]                                   =
      F.minOption(self)
    def minByOption[B: Ord](f: A => B)(implicit F: ParTraversable[F]): Option[A]                         =
      F.minByOption(self)(f)
    def nonEmpty(implicit F: ParTraversable[F]): Boolean                                                 =
      F.nonEmpty(self)
    def reduceAssociative(implicit F: ParTraversable[F], A: Associative[A]): Option[A]                   =
      F.reduceAssociative(self)
    def reduceIdempotent(implicit F: ParTraversable[F], ia: Idempotent[A], ea: Equal[A]): Option[A]      =
      F.reduceIdempotent(self)
    def reduceIdentity(implicit F: ParTraversable[F], A: Identity[A]): A                                 =
      F.reduceIdentity(self)
    def product(implicit A: Identity[Prod[A]], F: ParTraversable[F]): A                                  =
      F.product(self)
    def reduceMapOption[B: Associative](f: A => B)(implicit F: ParTraversable[F]): Option[B]             =
      F.reduceMapOption(self)(f)
    def reduceOption(f: (A, A) => A)(implicit F: ParTraversable[F]): Option[A]                           =
      F.reduceOption(self)(f)
    def reverse(implicit F: ParTraversable[F]): F[A]                                                     =
      F.reverse(self)
    def size(implicit F: ParTraversable[F]): Int                                                         =
      F.size(self)
    def sum(implicit A: Identity[Sum[A]], F: ParTraversable[F]): A                                       =
      F.sum(self)
    def toChunk(implicit F: ParTraversable[F]): Chunk[A]                                                 =
      F.toChunk(self)
    def zipWithIndex(implicit F: ParTraversable[F]): F[(A, Int)]                                         =
      F.zipWithIndex(self)
  }
  ...
  implicit class FlipOps[F[+_], G[+_], A](private val self: F[G[A]]) {
    def flip[B](implicit
      traversable: ParTraversable[F],
      identityBoth: IdentityBoth[G],
