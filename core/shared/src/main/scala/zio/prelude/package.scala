...
package object prelude
  ...
  implicit class AnySyntax[A](private val a: A) extends AnyVal {
    ...
    /**
     * Applies function `f` to an argument `a`, like `f(a)`, but in flipped order and doesn't need parentheses.
     * Can be chained, like `x |> f |> g` = `g(f(x))`.
     * Works like a pipe in shell.
     * See `|:` for the opposite way.
     */
    def |>[B](f: A => B): B = f(a)

    /**
     * Applies function `f` to an argument `a`, like `f(a)` and doesn't need parentheses.
     * Can be chained, like `g |: f |: x` = `g(f(x))`.
     * Works like a pipe in shell, but in right-to-left direction.
     * See `|>` for the opposite way.
     * Note, that by virtue of having `:` at the end, the operator is right-associative.
     */
    def |:[B](f: A => B): B = f(a)
